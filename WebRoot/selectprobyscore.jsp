...
<form class="am-form">
  ...
  <div class="am-form-group">
    ...
    <div class="am-form-group">
      <label for="pro">我的专业意向：（该列表的值通过Ajax从数据库获取）</label>
...
$(document).ready(function() {
	...
	$("#result").html("");//先清空原来的表格内容
		$.getJSON("${pageContext.request.contextPath}/servlet/SelectProByScoreServlet?"+
				"myScore="+$("#myScore").val()+
				"&area="+$("#area").val()+
				"&type="+$("#type").val()+
				"&batch="+$("#batch").val()+
			...
			$.each(json, function(i,item){
    			$("#result").append(
    				...
    				"<td>" + item.batch + "</td>" +
    				"<td>" + item.proCode + "</td>" +
    				"<td><a href='" + item.proIntro + "' target='_blank'>" + item.proName + "</a>" + "</td>" +
...
$(document).ready(function() {
	$.getJSON("${pageContext.request.contextPath}/servlet/ProRankingServlet?timeStamp="+new Date().getTime(), function(json) {
			$.each(json, function(i,proinfo){
    			$("#pro").append("<option value='" + proinfo.proCode + "'>" + proinfo.proName + "</option>");
  			});
		});
});
</script>
